...
kappa = 30;  % Condition number of the matrix
epsilon = 1/kappa; 
...
[A,~]=randMat_gen_gen(kappa,n_qubit,seed); 
b = 2*rand(dim,1)-1; 
b = b/norm(b); 
...
C = zeros(2*dim,2*dim);
C((dim+1):(2*dim),1:dim) = A';
C(1:dim,(dim+1):(2*dim)) = A; 

H0 = kron(sigmap,B*Pb) + kron(sigmam,Pb*B);
H1 = kron(sigmap,C*Pb) + kron(sigmam,Pb*C);

x_exact_n3 = kron(zero,kron(one,x_exact_n)); 

ini = kron(zero,kron(zero,b)); 
